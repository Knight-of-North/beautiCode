...
export function toChineseErrorMessage(value: unknown): string {
  ...
  const rules: Array<[RegExp, string | ((match: RegExpExecArray) => string)]> = [
    ...
    [/Video backgrounds must use an MP4 or MOV file\.?/i, "视频背景必须使用 MP4 或 MOV 文件。"],
    [/Video backgrounds must use an MP4 file\.?/i, "视频背景必须使用 MP4 或 MOV 文件。"],
    [/Video background must be a non-empty MP4 or MOV no larger than (\d+) bytes\.?/i, (match) => `视频背景必须是非空 MP4 或 MOV 文件，且不超过 ${match[1]} 字节。`],
    [/Video background must be a non-empty MP4 no larger than (\d+) bytes\.?/i, (match) => `视频背景必须是非空 MP4 或 MOV 文件，且不超过 ${match[1]} 字节。`],
    [/Video background is not a valid MP4 or MOV container\.?/i, "视频背景不是有效的 MP4 或 MOV 容器。"],
    [/Video background is not a valid MP4 container\.?/i, "视频背景不是有效的 MP4 或 MOV 容器。"],
